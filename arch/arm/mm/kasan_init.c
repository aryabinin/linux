...
static int __init creat_mapping(unsigned long start, unsigned long end, int node)
{
...
void __init kasan_init(void)
{
	...
	pr_info("clearing pgd\n");
	clear_pgds(KASAN_SHADOW_START, KASAN_SHADOW_END);

	pr_info("cleared pgd\n");
	kasan_populate_zero_shadow(kasan_mem_to_shadow(KASAN_SHADOW_START),
				kasan_mem_to_shadow(KASAN_SHADOW_END));

	kasan_populate_zero_shadow(kasan_mem_to_shadow(VMALLOC_START),
				kasan_mem_to_shadow(-1UL) + 1);
	...
	for_each_memblock(memory, reg) {
		...
		creat_mapping(kasan_mem_to_shadow(start), kasan_mem_to_shadow(end), NUMA_NO_NODE);
	}
