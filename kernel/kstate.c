		...
		void *obj, int section_type)
{
	...
	while (field->flags != KS_END) {
		...
		/* Do we need this ?? fields of higher versions shouldn't exist */
		if (field->version_id > kstate->version_id) {
			field++;
			continue;
		}
...
int kstate_save_state(void)
{
	...
	list_for_each_entry(se, &states, list) {
		ret = save_kstates(&kstate_stream, se->id, se->kstd, se->obj);
		if (ret)
			return ret;
	}
				...
				struct state_entry *se)
{
	...
	while (ke->section_type != KS_EOF) {
		...
		ke = (struct kstate_entry *)(ke->data + ke->size);
		subsection_load(&stream, se->id, se->kstd, se->obj);
	}
}
